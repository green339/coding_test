...
visited=[[[[] for _ in range(N*M)] for _ in range(M)]for _ in range(N)]
...
while q:
    ...
    for di in range(4):
        nrx,nry,nbx,nby=order(rx,ry,bx,by,dd[di])
        if nrx==-1:
            continue
        if visited[nrx][nry][nbx*M+nby]:
            continue
        visited[nrx][nry][nbx*M+nby]=[visited[rx][ry][bx*M+by][0]+1,visited[rx][ry][bx*M+by][1]+d[di]]
